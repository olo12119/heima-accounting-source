...
test('完整本地账本流程：收支、分类、导入备份、预算模板、主题与密码锁', async () => {
  ...
  try {
    ...
    await expect(page.getByRole('dialog', { name: '记一笔支出' })).toBeHidden()
    ...
    await page.getByRole('button', { name: '切换心情主题' }).click()
    await page.getByRole('dialog', { name: '心情主题' }).getByRole('button', { name: /暖阳活力/ }).click()
    ...
    await page.getByRole('button', { name: '保存支出' }).click()
    await expect(page.getByRole('dialog', { name: '编辑这笔支出' })).toBeHidden()
    ...
    await page.getByRole('button', { name: /深海专注/ }).click()
    await expect(page.locator('html')).toHaveAttribute('data-color-theme', 'ocean')
    await page.getByRole('button', { name: /深色/ }).click()
    await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark')
    await page.getByRole('button', { name: /暖阳活力/ }).click()
    await expect(page.locator('html')).toHaveAttribute('data-color-theme', 'amber')
    await page.getByRole('button', { name: /云朵治愈/ }).click()
    ...
    await expect(page.getByRole('dialog', { name: '记一笔收入' })).toBeHidden()
