...
import { createBackupDocument, createExpensesCsv, hashPayload, parseBackupDocument, parseExpensesCsv } from '../../src/main/data-formats'
...
describe('CSV 格式', () => {
  ...
  it('读取带引号和换行的标准账单 CSV', () => {
    const csv = createExpensesCsv([{ ...payload.expenses[0]!, primaryCategoryName: '餐饮', secondaryCategoryName: '正餐', primaryCategoryIcon: 'utensils', primaryCategoryColor: '#d98257', note: '午餐,套餐' }])
    const rows = parseExpensesCsv(csv)
    expect(rows).toHaveLength(1)
    expect(rows[0]).toMatchObject({ entryType: '支出', amount: '12.50', primaryCategory: '餐饮', secondaryCategory: '正餐', note: '午餐,套餐' })
  })
})
