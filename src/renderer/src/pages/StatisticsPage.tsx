...
import type { EntryType, StatisticsPreset } from '../../../shared/types'
import { CategoryIcon } from '../components/CategoryIcon'
import { AnimatedAmount } from '../components/AnimatedAmount'
import { getErrorMessage } from '../lib/errors'

const ranges: Array<{ value: StatisticsPreset; label: string }> = [
  { value: 'today', label: '今日' }, { value: 'week', label: '本周' }, { value: 'month', label: '本月' }, { value: 'year', label: '今年' }, { value: 'custom', label: '自定义' }
]
...
export function StatisticsPage(): React.JSX.Element {
  ...
  const query = useQuery({ queryKey: ['statistics', preset, entryType, startDate, endDate], queryFn: () => window.heima.queryStatistics({ preset, entryType, startDate: preset === 'custom' ? startDate : undefined, endDate: preset === 'custom' ? endDate : undefined }) })
  ...
  return <div className={`statistics-page ${entryType}`}>
    <div className="statistics-controls"><div><div className="segmented statistics-tabs">{ranges.map((range) => <button key={range.value} className={preset === range.value ? 'active' : ''} onClick={() => setPreset(range.value)}>{range.label}</button>)}</div>{preset === 'custom' && <div className="stats-custom-range"><input aria-label="统计开始日期" type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /><span>至</span><input aria-label="统计结束日期" type="date" value={endDate} onChange={(event) => setEndDate(event.target.value)} /></div>}</div><div className="stats-type-switch"><button className={entryType === 'expense' ? 'active expense' : ''} onClick={() => setEntryType('expense')}><ArrowUpRight size={15} />支出分析</button><button className={entryType === 'income' ? 'active income' : ''} onClick={() => setEntryType('income')}><ArrowDownLeft size={15} />收入分析</button></div></div>
    {query.isLoading ? <div className="stats-loading skeleton" /> : query.isError ? <div className="page-error">{getErrorMessage(query.error)}</div> : data && <>
      <motion.section key={`${entryType}-${preset}`} className="stats-hero" initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : 0.36 }}><div><span>这一阶段共{label}</span><AnimatedAmount cents={data.totalCents} /><small>{data.startDate} 至 {data.endDate}</small><em>{data.changePercent === null ? '上一周期暂无数据' : `较上一周期${data.changePercent >= 0 ? '增加' : '减少'} ${Math.abs(data.changePercent)}%`}</em></div><motion.div className="stats-hero-mark" animate={reduceMotion ? {} : { y: [0, -5, 0], rotate: [0, 3, 0] }} transition={{ duration: 3.6, repeat: Infinity, ease: 'easeInOut' }}><TrendingUp size={25} /></motion.div></motion.section>
