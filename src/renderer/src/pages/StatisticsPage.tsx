...
export function StatisticsPage(): React.JSX.Element {
  ...
  return <div className={`statistics-page ${entryType}`}>
    ...
    {query.isLoading ? <div className="stats-loading skeleton" /> : query.isError ? <div className="page-error">{getErrorMessage(query.error)}</div> : data && <>
      <section className="stats-hero"><div><span>这一阶段共{label}</span><strong>{formatCents(data.totalCents)}</strong><small>{data.startDate} 至 {data.endDate}</small><em>{data.changePercent === null ? '上一周期暂无数据' : `较上一周期${data.changePercent >= 0 ? '增加' : '减少'} ${Math.abs(data.changePercent)}%`}</em></div><div className="stats-hero-mark"><TrendingUp size={25} /></div></section>
