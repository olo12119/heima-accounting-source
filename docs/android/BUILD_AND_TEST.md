...
- JVM 测试：金额、连续月趋势、日期、分类、CSV、反馈门、版本比较、材质降级和真实财务洞察等 38 项。
- `connectedDebugAndroidTest`：在已启动的模拟器执行 40 项数据库迁移、UI、手势和截图测试。
- `assembleRelease`：生成 R8 压缩的未签名 Release 中间产物。

最终签名使用忽略 Git 的 `.local-signing/heima-release.jks`。密钥丢失后无法给现有用户做覆盖升级，因此必须单独备份。1.1.0 使用与 1.0.3 相同证书签名。
