...
export function DashboardPage(): React.JSX.Element {
  ...
  const enter = (delay: number) => reduceMotion ? {} : {
    ...
    transition: { delay, duration: 0.46, ease: [0.22, 1, 0.36, 1] as const }
  }

  return <div className="dashboard-v2">
    <motion.section className="money-hero" {...enter(0)}>
      <div className="money-hero-copy"><span className="hero-kicker"><Sparkles size={14} />本月资金视图</span><p>当前结余</p><AnimatedAmount cents={Math.abs(data.monthBalanceCents)} prefix={balanceNegative ? '−' : '+'} className={balanceNegative ? 'negative' : ''} /><div className="hero-flow"><span><ArrowDownLeft size={15} />收入 <b>+{formatCents(data.monthIncomeCents)}</b></span><span><ArrowUpRight size={15} />支出 <b>−{formatCents(data.monthExpenseCents)}</b></span></div><button onClick={openNewExpense}><Plus size={17} />快速记一笔</button></div>
      <div className="hero-chart"><div><span>最近7天资金流动</span><small>悬停查看每日金额</small></div><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.dailyCashFlow} margin={{ top: 12, right: 4, bottom: 0, left: 4 }}><defs><linearGradient id="incomeGlow" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--chart-income)" stopOpacity=".5"/><stop offset="100%" stopColor="var(--chart-income)" stopOpacity="0"/></linearGradient><linearGradient id="expenseGlow" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--chart-expense)" stopOpacity=".45"/><stop offset="100%" stopColor="var(--chart-expense)" stopOpacity="0"/></linearGradient></defs><XAxis dataKey="date" tickFormatter={(value) => String(value).slice(5)} axisLine={false} tickLine={false} tick={{ fill: 'var(--hero-chart-muted)', fontSize: 9 }} /><Tooltip formatter={(value) => formatCents(Number(value))} /><Area type="monotone" dataKey="incomeCents" name="收入" stroke="var(--chart-income)" strokeWidth={2.5} fill="url(#incomeGlow)" /><Area type="monotone" dataKey="expenseCents" name="支出" stroke="var(--chart-expense)" strokeWidth={2.5} fill="url(#expenseGlow)" /></AreaChart></ResponsiveContainer></div>
    </motion.section>

    <motion.div className="home-insight-grid" {...enter(0.08)}>
      <section className="insight-card expense"><span className="insight-orb"><ArrowUpRight size={20} /></span><div><small>本月支出</small><AnimatedAmount cents={data.monthExpenseCents} /><span>今日 −{formatCents(data.todayExpenseCents)}</span></div></section>
      <section className="insight-card income"><span className="insight-orb"><ArrowDownLeft size={20} /></span><div><small>本月收入</small><AnimatedAmount cents={data.monthIncomeCents} /><span>今日 +{formatCents(data.todayIncomeCents)}</span></div></section>
    ...
    <motion.div className="home-content-grid" {...enter(0.16)}><section className="panel recent-panel">
